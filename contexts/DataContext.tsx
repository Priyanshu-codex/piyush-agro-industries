...
import {
  ...
  EXTENDED_PRODUCTS as staticExtendedProducts,
  GALLERY_ITEMS as staticGalleryItems,
  FAQ_ITEMS as staticFaqItems,
  TESTIMONIALS as staticTestimonials,
  PRODUCT_CATEGORIES as staticCategories,
...
const MOCK_STORAGE_PREFIX = 'piyush_agro_mock_';
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const mutateStaticTranslations = (
    ...
    const activeProds = currentProducts.filter((p: any) => p.status !== 'hidden');
    staticExtendedProducts.length = 0;
    staticExtendedProducts.push(...activeProds);

    const featuredProds = activeProds.filter((p: any) => p.featured);
    staticProducts.length = 0;
    staticProducts.push(...featuredProds);

    const activeCats = currentCategories.filter((c: any) => c.status !== 'hidden');
    staticCategories.length = 0;
    staticCategories.push('All', ...activeCats.map((c) => c.name.en));
  ...
  const initializeSupabaseDefaults = async () => {
    try {
      const { saveProduct } = await import('@/services/productService');
      ...
      for (const p of staticExtendedProducts) {
        const isFeatured = staticProducts.some((sp) => sp.id === p.id);
        await saveProduct(p.id, { ...p, featured: isFeatured, status: 'active', displayOrder: 10 });
      }
      ...
      console.error('Error initializing Supabase defaults', e);
    }
  };
  ...
  useEffect(() => {
    ...
    if (isConfigured) {
      const loadServices = async () => {
        const supabase = createClient();
        const { subscribeProducts } = await import('@/services/productService');
        const { subscribeCategories } = await import('@/services/categoryService');
        const { subscribeGallery } = await import('@/services/galleryService');
        const { subscribeHomepageSettings, subscribeContactSettings, subscribeGeneralSettings } = await import('@/services/settingsService');
        ...
        // Testimonials
        supabase.from('testimonials').select('*').order('displayOrder', { ascending: true }).then(({ data }) => {
        ...
        const testChan = supabase.channel(`tests_chan_${Math.random()}`).on('postgres_changes', { event: '*', schema: 'public', table: 'testimonials' }, async () => {
          const { data } = await supabase.from('testimonials').select('*').order('displayOrder', { ascending: true });
          if (data) setTestimonials(data as Testimonial[]);
        }).subscribe();
      ...
      let cleanupPromise = loadServices();
      return () => {
        cleanupPromise.then((cleanup) => cleanup());
      };
    } else {
      const cachedProducts = getMockData('products', null);
      if (!cachedProducts) {
        setMockData('products', staticExtendedProducts.map((p, idx) => ({
          ...p,
          featured: staticProducts.some((sp) => sp.id === p.id),
          status: 'active',
          displayOrder: idx + 1,
        })));
  ...
  }, [products, categories, galleryItems, faqs, testimonials, homepageSettings, contactSettings, generalSettings]);
