...
function ProductDetailContent({ params }: PageProps) {
  ...
  const product = useMemo(() => {
    return products.find((p: any) => p.slug === params.id || p.id === params.id);
  }, [params.id, products]);

  const productCatName = useMemo(() => {
    if (!product) return '';
    const cat = categories.find((c: any) => c.id === product.category_id);
    return cat ? cat.name?.en : 'Unknown Category';
  }, [product, categories]);
  ...
  const relatedProducts = useMemo(() => {
    ...
    return products.filter(
      (p: any) => p.category_id === product.category_id && p.id !== product.id
    ).slice(0, 3);
  }, [product, products]);
