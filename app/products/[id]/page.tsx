...
import { t, PRODUCTS, EXTENDED_PRODUCTS } from '@/lib/translations';
...
// Sidebar categories with precise counts and search params mapping
const SIDEBAR_CATEGORIES = [
  { name: { en: 'Generator Trailers', hi: 'जनरेटर ट्रेलर' }, count: 3, query: '/products?category=Generator Trolley' },
  { name: { en: 'Tractor Trailers', hi: 'ट्रैक्टर ट्रेलर' }, count: 7, query: '/products?category=Tractor Trailers' },
  { name: { en: 'Hydraulic Tractor Trolley', hi: 'हाइड्रोलिक ट्रैक्टर ट्रॉली' }, count: 3, query: '/products?category=Hydraulic Tractor Trolley' },
  { name: { en: 'Generator Trolley', hi: 'जनरेटर ट्रॉली' }, count: 4, query: '/products?category=Generator Trolley' },
  { name: { en: 'Material Handling Equipment', hi: 'मटेरियल हैंडलिंग उपकरण' }, count: 4, query: '/products?category=Material Handling Equipment' },
  { name: { en: 'Industrial Automotive Trailer', hi: 'औद्योगिक ऑटोमोटिव ट्रेलर' }, count: 3, query: '/products?category=Material Handling Equipment' },
  { name: { en: 'Water Tanker Trailers', hi: 'वाटर टैंकर ट्रेलर' }, count: 5, query: '/products?category=All&search=water' },
  { name: { en: 'Hydraulic Tipping Trailer', hi: 'हाइड्रोलिक टिपिंग ट्रेलर' }, count: 2, query: '/products?category=Tractor Trailers&search=tipping' },
  { name: { en: 'Heavy Duty Flatbed Trailers', hi: 'हैवी ड्यूटी फ्लैटबेड ट्रेलर' }, count: 3, query: '/products?category=Material Handling Equipment&search=low' },
  { name: { en: 'Side Wall Trailer', hi: 'साइड वॉल ट्रेलर' }, count: 1, query: '/products?category=Tractor Trailers' },
  { name: { en: 'Jeep Trailers', hi: 'जीप ट्रेलर' }, count: 1, query: '/products?category=Tractor Trailers' },
  { name: { en: '4 Wheeler Trailer', hi: '4 व्हीलर ट्रेलर' }, count: 1, query: '/products?category=Tractor Trailers' },
  { name: { en: 'Heavy Equipment Trailers', hi: 'भारी उपकरण ट्रेलर' }, count: 1, query: '/products?category=Material Handling Equipment' },
  { name: { en: 'Agriculture', hi: 'कृषि' }, count: 2, query: '/products?category=All&search=agri' }
];

// Helper to combine all products
const ALL_PRODUCTS = [...PRODUCTS, ...EXTENDED_PRODUCTS];
...
function ProductDetailContent({ params }: PageProps) {
  ...
  const product = useMemo(() => {
    return ALL_PRODUCTS.find((p) => p.id === params.id);
  }, [params.id]);
  ...
  const galleryItems = useMemo(() => {
    if (!product) return [];
    return [
      { icon: product.icon, gradient: product.gradient },
      { icon: product.icon, gradient: product.gradient.replace('from-', 'to-').replace('to-', 'from-') }, // Swapped
      { icon: product.icon, gradient: 'from-[#111827] to-[#1f2937]' }, // Dark premium mode
    ];
  }, [product]);
      ...
      <section className="bg-white border-b border-gray-100 py-4 shadow-sm relative z-20">
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap items-center justify-between gap-4 text-xs sm:text-sm">
          <div className="flex items-center gap-1.5 sm:gap-2 text-gray-500 font-medium">
            ...
            <ChevronRight size={14} className="text-gray-400" />
            <span className="text-gray-400 max-w-[150px] sm:max-w-none truncate">{product.category}</span>
            <ChevronRight size={14} className="text-gray-400" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <aside className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-card p-5 sticky top-[120px] hidden lg:block">
            ...
            <ul className="space-y-1">
              {SIDEBAR_CATEGORIES.map((cat, idx) => {
                const isActive = product.category.toLowerCase().includes(cat.name.en.toLowerCase().replace(' trailers', '').replace(' trolley', '')) || 
                                 (cat.name.en === 'Agriculture' && product.category === 'agri');
                return (
                  <li key={idx}>
                    <button
          ...
          <article className="lg:col-span-5 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-card space-y-4">
              ...
              <div 
                className={`relative overflow-hidden w-full h-72 sm:h-96 rounded-xl bg-gradient-to-br ${galleryItems[activeImgIdx]?.gradient || 'from-primary to-primary-dark'} flex items-center justify-center cursor-zoom-in shadow-inner border border-white/10`}
              ...
              >
                ...
                >
                  {galleryItems[activeImgIdx]?.icon}
                </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {galleryItems.map((item, idx) => (
                  <button
                    ...
                    className={`h-16 sm:h-20 rounded-xl bg-gradient-to-br ${item.gradient} flex items-center justify-center text-3xl transition-all relative overflow-hidden border-2 ${
                      activeImgIdx === idx 
                ...
                <table className="w-full text-left border-collapse text-xs sm:text-sm">
                  <tbody>
                    ...
                    {product.specs && (
                      <>
                        <tr className="bg-gray-50/70 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="p-3 font-semibold text-gray-700 w-1/3 border-r border-gray-100">{lang === 'en' ? 'Component Part' : 'पार्ट का नाम'}</td>
                          <td className="p-3 text-gray-600">{product.specs.nameOfPart}</td>
                        </tr>
                        <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="p-3 font-semibold text-gray-700 w-1/3 border-r border-gray-100">{lang === 'en' ? 'Loading Capacity' : 'लोडिंग क्षमता'}</td>
                          <td className="p-3 text-gray-600 font-semibold text-primary">{product.specs.capacity}</td>
                        </tr>
                        <tr className="bg-gray-50/70 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="p-3 font-semibold text-gray-700 w-1/3 border-r border-gray-100">{lang === 'en' ? 'Dimensions (L×B×H)' : 'आयाम (लंबाई×चौड़ाई×ऊंचाई)'}</td>
                          <td className="p-3 text-gray-600">{product.specs.size}</td>
                        </tr>
                      </>
                    )}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedProducts.map((p) => (
                <div 
                  ...
                  className="card-hover bg-white rounded-2xl overflow-hidden shadow-card border border-gray-100 flex flex-col cursor-pointer"
                >
                  <div className={`h-36 shrink-0 bg-gradient-to-br ${p.gradient} flex items-center justify-center relative overflow-hidden`}>
                    ...
                    <span className="text-5xl relative z-10 drop-shadow">{p.icon}</span>
                  </div>
                  <div className="p-5 flex flex-col flex-1" onClick={(e) => e.stopPropagation()}>
                    <span className="text-[10px] uppercase font-bold tracking-widest text-primary mb-1">{p.category}</span>
                    <h3 className="font-bold font-rajdhani text-gray-900 text-base mb-2 line-clamp-1 break-words">{tx(p.title)}</h3>
                    ...
                    <div className="flex gap-2.5 mt-auto pt-3 border-t border-gray-100/60">
                      ...
                      <button
                        onClick={() => openEnquiry(tx(p.title))}
